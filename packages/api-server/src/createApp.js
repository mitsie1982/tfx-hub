...
const ADMIN_WHATSAPP_DISABLED_MESSAGE = 'Admin WhatsApp access is disabled. Use the browser or mobile admin workspace with your secret username and admin password.';
...
function buildAdminAuditCsv(items = []) {
  const headers = ['id', 'adminUserId', 'eventType', 'outcome', 'reason', 'identifier', 'requestPath', 'requestMethod', 'targetUserId', 'ipAddress', 'createdAt'];
...
function summarizeWhatsappSubscription(user) {
  return {
    ...
    nextStep: user.phoneNumber ? `Send HI from ${user.phoneNumber} on WhatsApp to continue onboarding.` : 'Enter a mobile number to subscribe.'
  };
}
...
function createApp({ repository, logger, metaWebhookOptions, adminAccessPolicy, getCurrentDate } = {}) {
  ...
  const metaOnboardingService = {
    async handleIncomingMessage({ phoneNumber, message }) {
      ...
      if (['admin', 'ams'].includes(command)) {
        await repository.upsertWhatsappMetaRouterSession(normalizedPhone, { selectedRole: null, screen: 'role_select' });
        return {
          reply: ADMIN_WHATSAPP_DISABLED_MESSAGE,
          options: [],
      ...
      if (!selectedRole) {
        return {
          reply: 'Reply 1 for Contractor, 2 for Customer, 3 for Association, or 4 for Professional. Admin WhatsApp access is disabled.',
          options: ['1', '2', '3', '4'],
          session: { phoneNumber: normalizedPhone, linked: false, screen: 'role_select', userId: null }
        };
      }
  ...
  function requireTenantAccess() {
    return [auth.tenantMiddleware(), enforceAdminAccessWindow];
  }
  ...
  app.post('/auth/register', async (req, res) => {
    ...
    if (!email || !password || !firstName || !lastName || !role) {
      return res.status(400).json({ error: 'validation_error', message: 'email, password, firstName, lastName, and role are required' });
    }
  ...
  app.post('/auth/password-reset/request', async (req, res) => {
    ...
    if (user.role === 'admin' && user.isBootstrapAdmin) {
      ...
      return res.status(403).json({ error: 'bootstrap_admin_reset_forbidden', message: 'Bootstrap admin reset must be handled out-of-band.' });
    }
  ...
  app.get('/admin/accounts', ...requireTenantAccess(), requireAdminRole, async (req, res) => {
    const items = typeof repository.listAdminUsers === 'function'
    ...
    return res.json({ items: items.map(userPayload), meta: { associationId: req.association.id, requester: req.auth.sub } });
  });
  ...
  app.post('/admin/accounts', ...requireTenantAccess(), requireAdminRole, async (req, res) => {
    ...
    if (!email || !password || !firstName || !lastName || !username) {
      return res.status(400).json({ error: 'validation_error', message: 'email, username, password, firstName, and lastName are required' });
    }
    ...
    return res.status(201).json({ item: userPayload(item), meta: { associationId: req.association.id, requester: req.auth.sub } });
  });
  ...
  app.post('/admin/accounts/:id/rotate-credentials', ...requireTenantAccess(), requireAdminRole, async (req, res) => {
    ...
    if (target.isBootstrapAdmin) {
      ...
      return res.status(403).json({ error: 'bootstrap_admin_rotation_forbidden', message: 'Bootstrap admin credentials must be rotated out-of-band.' });
    }
    ...
    const item = typeof repository.updateAdminUserCredentials === 'function'
  ...
  app.post('/admin/accounts/:id/password-reset-request', ...requireTenantAccess(), requireAdminRole, async (req, res) => {
    const target = await repository.getUserById(req.params.id);
    if (!target || target.role !== 'admin') {
      return res.status(404).json({ error: 'admin_user_not_found' });
    }
    ...
    return res.json({ ok: true, targetUserId: target.id, expiresAt: entry.expiresAt, resetToken: delivery.resetToken, delivery });
  });

  app.get('/admin/audit-events', ...requireTenantAccess(), requireAdminRole, async (req, res) => {
    const items = typeof repository.listAdminAuditEvents === 'function'
  ...
  app.get(['/jobs', '/orders'], ...requireTenantAccess(), async (req, res) => {
    ...
    return res.json({ items: items.map(buildJobSummary), meta: { associationId: req.association.id, requester: req.auth.sub } });
  });
