...
function createWhatsappContractorApi(client, context = {}) {
  ...
  return {
    async getSession(phoneNumber) {
      ...
      const response = await client.get(`/whatsapp/contractor/session/${encodeURIComponent(phoneNumber)}`, withContext());
    ...
    async sendMessage(phoneNumber, message) {
      ...
      const response = await client.post('/whatsapp/contractor/messages', { phoneNumber, message }, withContext());
      return response.data;
    }
  };
}
