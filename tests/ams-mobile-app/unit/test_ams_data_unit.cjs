...
async function run() {
  ...
  const detail = await amsData.fetchAdminContractorDetail('pro-003');
  assert.strictEqual(detail.item.name, 'Theuns Fraser');
  ...
  const action = await amsData.performAdminContractorAction('pro-003', 'tier-review');
