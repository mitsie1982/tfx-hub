...
async function waitForServerReady() {
  ...
  throw new Error(`Integration server did not become ready on ${baseURL}`);
}
...
(async () => {
  try {
    ...
    assert.ok(whatsappMenuResponse.reply.includes('Completed jobs: 128'));
    ...
    assert.ok(whatsappLeadsResponse.reply.includes('Showing 1-3 of'));
    assert.ok(whatsappLeadsResponse.reply.includes('Reply MORE for the next leads'));
    assert.ok(whatsappMoreLeadsResponse.reply.includes('Showing 4-'));
    assert.ok(whatsappMoreLeadsResponse.reply.includes('Roof waterproofing and ceiling patch repair'));
    ...
    assert.ok(whatsappCustomerMenu.reply.includes('Welcome back, Michelle'));
