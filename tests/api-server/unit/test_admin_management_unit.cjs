...
async function run() {
  ...
  try {
    ...
    const overrideServer = await new Promise((resolve) => {
      const instance = overrideApp.listen(0, () => resolve(instance));
    });

    try {
      const overrideBaseUrl = `http://127.0.0.1:${overrideServer.address().port}`;
      const overrideResponse = await fetch(`${overrideBaseUrl}/auth/login`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ identifier: 'ops.supervisor', password: 'rotated-password-456' })
      });
      const overridePayload = await overrideResponse.json();
      assert.strictEqual(overrideResponse.status, 200);
      assert.strictEqual(overridePayload.user.id, createdAdminId);
    } finally {
    ...
    const csvAuditResponse = await fetch(`${baseUrl}/admin/audit-events?limit=20&format=csv&outcome=success`, {
      headers: {
        authorization: `Bearer ${token}`,
        'x-association-id': 'assoc-contractor-demo'
      }
