...
async function run() {
  ...
  try {
    ...
    assert.ok(contractorHtml.includes('Create Contractor Profile'));
    ...
    assert.ok(customerHtml.includes('Create Customer Profile'));
    ...
    assert.ok(contractorLoginHtml.includes('Contractor Browser'));

    const contractorRegisterHtml = await postForm(contractor, '/actions/register', {
      firstName: 'Lindiwe',
      lastName: 'Dube',
      trade: 'electrician',
      phoneNumber: '+27710000123',
      email: 'lindiwe.dube@example.com',
      password: 'password123'
    });
    assert.ok(
      contractorRegisterHtml.includes('Profile created successfully')
      || contractorRegisterHtml.includes('Live Session Active')
      || contractorRegisterHtml.includes('Unable to register contractor account.')
    );

    const contractorRegisterValidationHtml = await postForm(contractor, '/actions/register', {
      firstName: 'Lindiwe',
      lastName: 'Dube',
      trade: 'electrician',
      phoneNumber: '',
    ...
    assert.ok(contractorRegisterValidationHtml.includes('Phone number is required.'));
    assert.ok(contractorRegisterValidationHtml.includes('lindiwe.dube@example.com'));

    const customerLoginHtml = await postForm(customer, '/actions/login', {
      email: 'client@example.com',
      password: 'password123'
    });
    assert.ok(customerLoginHtml.includes('Customer Browser'));

    const customerRegisterHtml = await postForm(customer, '/actions/register', {
      firstName: 'Michelle',
      lastName: 'Brummer',
      phoneNumber: '+27719990112',
      email: 'new-client@example.com',
      password: 'password123'
    });
    assert.ok(
      customerRegisterHtml.includes('Profile created successfully')
      || customerRegisterHtml.includes('Live Session Active')
      || customerRegisterHtml.includes('Unable to register a customer account.')
    );

    const customerRegisterValidationHtml = await postForm(customer, '/actions/register', {
      firstName: 'Michelle',
      lastName: 'Brummer',
      phoneNumber: '',
      email: 'new-client@example.com',
    ...
    assert.ok(customerRegisterValidationHtml.includes('Phone number is required.'));
    assert.ok(customerRegisterValidationHtml.includes('new-client@example.com'));
