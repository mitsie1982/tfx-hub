...
async function run() {
      ...
      return {
        session: {
          async register(payload) {
            ...
            return { user: { id: 'user-123', email: payload.email } };
          }
        }
      };
    }
  ...
  const registered = await contractorData.registerContractor({
    email: 'new-contractor@example.com',
    password: 'password123',
    firstName: 'New',
    lastName: 'Contractor',
    trade: 'electrician',
    phoneNumber: '+27710000999'
  });
  assert.strictEqual(registered.user.email, 'new-contractor@example.com');
  assert.strictEqual(registerPayload.phoneNumber, '+27710000999');
