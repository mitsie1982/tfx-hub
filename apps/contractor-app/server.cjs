// Restored loadModel function
function loadModel(projectsResult, selectedProjectId, profileResult, warning, historyResult, url, pendingAction) {
	...
	return {
		...
		projects,
		notice: null,
		registrationDraft: buildContractorRegistrationDraft(url),
		interactionDrafts: buildContractorInteractionDrafts(selectedProject),
...
function renderModel(model) {
		try {
			...
			return renderShell({
				...
				sections: [
					...
					renderSection('Contractor Profile', renderKeyValueTable([
						...
						<div class="form-grid">
							<div class="form-card">
								<h3>Create Contractor Profile</h3>
								<p class="muted">Register a contractor from the desktop demo and persist the profile through the API.</p>
								<form method="post" action="/actions/register">
									<input type="text" name="firstName" value="${escapeHtml(model.registrationDraft.firstName)}" placeholder="Theuns" required>
									<input type="text" name="lastName" value="${escapeHtml(model.registrationDraft.lastName)}" placeholder="Fraser" required>
									<input type="text" name="trade" value="${escapeHtml(model.registrationDraft.trade)}" placeholder="electrician" required>
									<input type="tel" name="phoneNumber" value="${escapeHtml(model.registrationDraft.phoneNumber)}" placeholder="+27710000001" required>
									<input type="email" name="email" value="${escapeHtml(model.registrationDraft.email)}" placeholder="contractor@example.com" required>
							...
							<div class="form-card">
								...
								<form method="post" action="/actions/login">
									<input type="text" name="identifier" value="${escapeHtml(model.interactionDrafts.login.identifier)}" placeholder="contractor@example.com or +27710000001" required>
							...
							<div class="form-card">
								...
								<form method="post" action="/actions/subscribe-whatsapp">
									<input type="tel" name="phoneNumber" value="${escapeHtml(model.interactionDrafts.whatsapp.phoneNumber)}" placeholder="+27710000001" required>
					...
					renderSection('Selected Lead Detail', selectedProject ? [
						...
						`<div class="form-grid">
							...
							<div class="form-card">
								...
								<form method="post" action="/actions/quote">
									<input type="hidden" name="projectId" value="${selectedProject.id}">
									<input type="text" name="amount" value="${escapeHtml(model.interactionDrafts.quote.amount)}" placeholder="R12,500" required>
									<input type="text" name="timeline" value="${escapeHtml(model.interactionDrafts.quote.timeline)}" placeholder="3 working days">
									<textarea name="note" placeholder="Brief note for the homeowner">${escapeHtml(model.interactionDrafts.quote.note)}</textarea>
							...
							<div class="form-card">
								...
								<form method="post" action="/actions/message">
									<input type="hidden" name="projectId" value="${selectedProject.id}">
									<textarea name="body" placeholder="I can inspect this tomorrow morning." required>${escapeHtml(model.interactionDrafts.message.body)}</textarea>
			...
			console.error('[renderModel] error:', err && err.stack ? err.stack : err);
			throw err;
		}
}
...
async function handlePost(requestUrl, form) {
	...
	if (url.pathname === '/actions/register') {
		...
		await contractorData.registerContractor({
			firstName: String(form.firstName || '').trim(),
			lastName: String(form.lastName || '').trim(),
			email: String(form.email || '').trim(),
			trade: String(form.trade || '').trim(),
			phoneNumber: String(form.phoneNumber || '').trim(),
			password: form.password
		});
		return '/?notice=' + encodeURIComponent('Profile created successfully. Contractor session is now active.');
	}
...
// Migrated from server.js to server.cjs for CommonJS clarity
// ...existing code from server.js will be moved here...
