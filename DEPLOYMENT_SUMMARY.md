...
**Note:** Integration with Airflow, Vertex Pipelines, MLMD (ML Metadata), and TF-Serving is **not present** in this pipeline. This is a gap for production TFX patterns and should be addressed if your use case requires native orchestration, metadata tracking, or model serving. See TFX documentation for integration options.

#### Production Gaps (TFX Orchestration, Metadata, Serving)

- ❌ **Airflow/Vertex Pipelines:** Not integrated. No DAG-based orchestration or managed pipeline execution. Manual and shell-based orchestration only.
- ❌ **MLMD (ML Metadata):** Not integrated. No lineage, artifact tracking, or metadata store for pipeline runs.
- ❌ **TF-Serving:** Not integrated. No automated model deployment or serving endpoint management.

**Impact:**

- These are required for full TFX production compliance (MLOps best practices, reproducibility, auditability, automated model serving).
- Current pipeline is suitable for E2E validation, not for managed ML workflow or model lifecycle.

**Roadmap:**

- Integrate Airflow/Vertex for pipeline orchestration (DAGs, scheduling, retries).
- Add MLMD for metadata tracking and lineage.
- Add TF-Serving or Vertex AI for model deployment and serving.

---

### TFX Hub Native Orchestration (Current State)

The current orchestration is implemented via shell scripts and GitHub Actions:

- `scripts/verify_full_pipeline.sh` — Main orchestrator (sequential stages)
- `trigger_alert_test.sh`, `simulate_metric_spike.sh`, `canary_launchdarkly.sh`, `run_staged_rollout_test.sh`, `dry_run_release.sh` — Modular helpers
- GitHub Actions workflow triggers and logs all stages

**No TFX pipeline objects, DAGs, or MLMD integration are present.**

---

**To achieve TFX-native orchestration:**

1. Refactor pipeline logic into TFX pipeline components (Python).
2. Define pipeline DAG using TFX DSL.
3. Deploy pipeline to Airflow or Vertex Pipelines.
4. Integrate MLMD for metadata tracking.
5. Add TF-Serving deployment step for trained models.

See TFX documentation for migration steps and examples.
...
For | See
──────────────────────────────────────────────────────────
Complete guide & troubleshooting | verify*full_pipeline_guide.md
...
Metric instrumentation | metrics_instrumentation*{backend,mobile}.md
