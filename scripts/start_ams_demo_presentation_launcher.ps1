...
Write-Host "Log file: C:\Users\1hans\tfx-hub\artifacts\demo-logs\start_ams_demo_launch_20260401T133659.log"
...
try {
  Start-Process -FilePath powershell -ArgumentList '-NoProfile','-ExecutionPolicy','Bypass','-WindowStyle','Normal','-Command', "& { & `"C:\Users\1hans\tfx-hub\scripts\start_ams_demo.ps1`" -Mode $Mode }" -WorkingDirectory 'C:\Users\1hans\tfx-hub\scripts' -WindowStyle Normal -PassThru | Out-Null
} catch {
...
<body style='font-family:Segoe UI,Arial;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;background:#f7f9fc'>
  <div style='padding:28px;border-radius:8px;background:white;box-shadow:0 6px 18px rgba(0,0,0,0.08);max-width:900px;text-align:center'>
    ...
    <p>Application: start_ams_demo</p>
