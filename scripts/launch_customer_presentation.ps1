# scripts/launch_customer_presentation.ps1
param(
  [int]$Port = 3000,
...
$AppPathCandidates = @('packages\customer-app','apps\customer-app')
$AppPath = $AppPathCandidates | ForEach-Object { Join-Path $RepoRoot $_ } | Where-Object { Test-Path $_ } | Select-Object -First 1
if (-not $AppPath) { $AppPath = Join-Path $RepoRoot 'packages\customer-app'; New-Item -ItemType Directory -Path $AppPath -Force | Out-Null }
...
function Ensure-Placeholder {
  ...
  if (-not (Test-Path $pkg)) {
    @{ name='customer-app'; version='0.0.1'; private=$true; scripts=@{ start='node server.js' } } | ConvertTo-Json -Depth 4 | Out-File -FilePath $pkg -Encoding UTF8
  }
  if (-not (Test-Path $srv)) {
    @"
const http = require('http');
const port = process.env.PORT || $port;
console.log('Customer placeholder server starting on port', port);
const server = http.createServer((req,res)=>{ res.writeHead(200,{'Content-Type':'text/html'}); res.end('<h1>Customer Demo</h1>'); });
...
if (Wait-ForUrl -url $Url -timeout $WaitTimeoutSeconds -interval $PollIntervalSeconds) {
  ...
  $present = Join-Path $AppPath 'demo_presentation_customer.html'
  "<html><body style='font-family:Segoe UI,Arial;text-align:center;padding:40px'><h1>Customer (no server)</h1></body></html>" | Out-File -FilePath $present -Encoding UTF8 -Force
