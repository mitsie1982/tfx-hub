# scripts/launch_contractor_presentation.ps1
param(
  [int]$Port = 3001,
...
$AppPathCandidates = @('packages\contractor-app','apps\contractor-app')
$AppPath = $AppPathCandidates | ForEach-Object { Join-Path $RepoRoot $_ } | Where-Object { Test-Path $_ } | Select-Object -First 1
if (-not $AppPath) { $AppPath = Join-Path $RepoRoot 'packages\contractor-app'; New-Item -ItemType Directory -Path $AppPath -Force | Out-Null }
...
function Ensure-Placeholder {
  ...
  if (-not (Test-Path $pkg)) {
    @{ name='contractor-app'; version='0.0.1'; private=$true; scripts=@{ start='node server.js' } } | ConvertTo-Json -Depth 4 | Out-File -FilePath $pkg -Encoding UTF8
  }
...
if (Wait-ForUrl -url $Url -timeout $WaitTimeoutSeconds -interval $PollIntervalSeconds) {
  ...
  $present = Join-Path $AppPath 'demo_presentation_contractor.html'
  "<html><body style='font-family:Segoe UI,Arial;text-align:center;padding:40px'><h1>Contractor (no server)</h1></body></html>" | Out-File -FilePath $present -Encoding UTF8 -Force
