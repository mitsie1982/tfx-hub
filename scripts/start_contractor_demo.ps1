# scripts/start_contractor_demo.ps1
param(
  [string]$Mode = 'dev'  # dev or release
)
$RepoRoot = 'C:\Users\1hans\tfx-hub'
$AppDir = Join-Path $RepoRoot 'packages/contractor-app'
if (-not (Test-Path $AppDir)) {
  Write-Host "Warning: app directory not found: $AppDir"
  Write-Host "Adjust path in scripts/start_contractor_demo.ps1"
  exit 1
}
Write-Host "Starting demo for contractor from $AppDir (Mode=$Mode)"

# Ensure the app is a real project (has package.json)
if (-not (Test-Path (Join-Path $AppDir 'package.json'))) {
  Write-Warning "No package.json found in $AppDir."
  Write-Host "The contractor-app is a placeholder. Initialize it first:"
  Write-Host "  cd $AppDir && npx react-native init ContractorApp (or your setup command)"
  exit 1
}
