	...
	context.subscriptions.push(vscode.commands.registerCommand('orchestrator.open', async () => {
			...
			return `
				...
				<head>
					...
					<style>
						...
						.var-form { margin: 1em 0; padding: 0.5em; background: #f9f9f9; border-radius: 4px; }
						.var-form label { display: block; margin-bottom: 0.2em; }
						.var-form input { margin-bottom: 0.5em; width: 100%; }
				...
				<body>
					...
					<div id="varFormModal" style="display:none;"></div>
					<script>
						...
						window.addEventListener('message', event => {
							...
							if (msg.type === 'workflows') {
								...
								workflows.forEach((wf, idx) => {
									...
									div.innerHTML = `<b>${wf.name}</b><br>${wf.description || ''}<br><button onclick="showVarForm(${idx})">Run</button>`;
									container.appendChild(div);
								});
							}
						});
						function showVarForm(idx) {
							selectedIdx = idx;
							const wf = workflows[idx];
							const modal = document.getElementById('varFormModal');
							if (!wf.variables || Object.keys(wf.variables).length === 0) {
								// No variables, run directly
								vscode.postMessage({ command: 'runWorkflow', workflow: wf, overrideVars: {} });
								return;
							}
							let html = `<div class='var-form'><h3>Edit Variables for ${wf.name}</h3><form id='varForm'>`;
							for (const [k, v] of Object.entries(wf.variables)) {
								html += `<label for='var_${k}'>${k}</label><input id='var_${k}' name='${k}' value='${v}' required />`;
							}
							html += `<div id='varFormError' class='error'></div><button type='submit'>Run</button> <button type='button' onclick='hideVarForm()'>Cancel</button></form></div>`;
							modal.innerHTML = html;
							modal.style.display = 'block';
							document.getElementById('varForm').onsubmit = function(e) {
								e.preventDefault();
								const data = {};
								let valid = true;
								let error = '';
								for (const [k, v] of Object.entries(wf.variables)) {
									const val = document.getElementById('var_' + k).value;
									if (val.trim() === '') {
										valid = false;
										error = `Variable '${k}' is required.`;
										break;
									}
								...
								if (!valid) {
									document.getElementById('varFormError').innerText = error;
									return;
								}
								modal.style.display = 'none';
								vscode.postMessage({ command: 'runWorkflow', workflow: wf, overrideVars: data });
							};
						}
						function hideVarForm() {
							document.getElementById('varFormModal').style.display = 'none';
						}
		...
		panel.webview.onDidReceiveMessage(async message => {
			if (message.command === 'refresh') {
				...
				// message.workflow and message.overrideVars are always present from the UI
